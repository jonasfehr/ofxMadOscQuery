...
bool ofxMadOscQuery::setupPageFromJson(std::list<MadParameterPage> &pages, MadParameterPage & page, ofxMidiDevice* midiDevice, ofJson element, string keyType){
    ...
    for(auto& contents : element["CONTENTS"]){
        ...
        // for MM33
//#ifdef MM33
        if(contents["DESCRIPTION"] == "Surface Parameters"){
            for(auto& parameters : contents["CONTENTS"]){
                if(parameters["TYPE"]=="f"){
                    page.addParameter(createParameter(parameters));
                }
            }
        }
        
        if(contents["DESCRIPTION"] == "Green"){
            page.addParameter(createParameter(contents));
        }
        if(contents["DESCRIPTION"] == "Blue"){
            page.addParameter(createParameter(contents));
        }
        if(contents["DESCRIPTION"] == "Red"){
            page.addParameter(createParameter(contents));
        }
//#endif
