...
void ofxMadOscQuery::iterateFind(ofJson json, string key, MadParameterPage *customPage, ofJson jsonSkipKeys)
{
    ...
    while (j < keySeg.size() && i < pathSeg.size())
    {
        ...
        else if (keySeg[j] == "*")
        {
            ...
            if (keySeg.size() - 1 != j)
            { // make sure there is more to come
                ...
                if (foundPos != std::string::npos)
                {
                    ...
                    if (path[endPos] == '/' || endPos == path.size())
                    { // check if at the end or has a "/" to follow
                        while (pathSeg[i] != keySeg[j + 1])
                        {
                            i++;
                        }
                        if (i == j)
                            isKeyCompatible = false; // if the key is at the spot of the * the pass...
                    }
                    else
                    {
                        isKeyCompatible = false;
...
void ofxMadOscQuery::createSubPages(std::list<MadParameterPage> &pages, ofxMidiDevice *midiDevice, ofJson json)
{
    ...
    for (auto &keyType : keyTypes)
    {
        for (auto &element : json["CONTENTS"][keyType]["CONTENTS"])
        {
            ...
            auto skipDescriptions = {"Next", "Per Type Selection", "Previous", "Select", "Select By Name", "Selected", "selected", "children"}; // Descriptions for surfaces, medias & fixtures
...
void ofxMadOscQuery::setupPageFromJson(std::list<MadParameterPage> &pages, MadParameterPage &page, ofxMidiDevice *midiDevice, ofJson element, string keyType)
{
    ...
    {
        if (contents["DESCRIPTION"].is_null() || !contents["DESCRIPTION"].is_string())
            continue;
