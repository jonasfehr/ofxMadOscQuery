...
void ofxMadOscQuery::iterateFind(ofJson &jsonReturn, const ofJson &json, const string &key, const ofJson &jsonSkipKeys)
{
	...
	for (size_t i = 0; i < keySeg.size() && i < pathSeg.size(); i++)
	{
		...
		else if (keySeg[i] == "*")
		{
			...
			{
				if (pathSeg[i] == skipKey.get<std::string>()) { isKeyCompatible = false; }
			}
		}
		else { isKeyCompatible = false; }
	}
	...
	while (j < (int)keySeg.size() && i < (int)pathSeg.size())
	{
		...
		else if (keySeg[j] == "*")
		{
			for (auto &skipKey : jsonSkipKeys)
			{
				for (int n = j; n <= i; n++)
				{
					if (pathSeg[n] == skipKey.get<std::string>()) { isKeyCompatible = false; }
				}
			}
			...
			if ((int)keySeg.size() - 1 != j)
			{
				...
				if (foundPos != std::string::npos)
				{
					...
					{
						while (i < (int)pathSeg.size() && pathSeg[i] != keySeg[j + 1]) { i++; }
			...
			{
				if ((int)pathSeg.size() == (int)keySeg.size() && isKeyCompatible) { j = (int)keySeg.size(); }
			}
			j++;
		}
		else { isKeyCompatible = false; break; }
	}
...
map<string, ofJson> ofxMadOscQuery::getContentMap(const ofJson &json, const string &key, const vector<string> &skipKeys)
{
	...
	for (auto it = container.begin(); it != container.end(); ++it)
	{
		...
		if (keySeg.size() > 2 && keySeg[2] == "*")
		{
			...
			{
				if (it.key() == skipKey) { isKeyCompatible = false; }
			}
		}
		else { isKeyCompatible = false; }

		if (isKeyCompatible) { contentMap[it.key()] = it.value(); }
	}
...
void ofxMadOscQuery::getConnectedMediaName(string *mediaName, const ofJson &json, const string &key, const ofJson &jsonSkipKeys)
{
	...
	while (j < (int)keySeg.size() && i < (int)pathSeg.size())
	{
		...
		else { isKeyCompatible = false; j++; i++; }
	}
	auto itType = json.find("TYPE");
...
void ofxMadOscQuery::createCustomPage(std::list<MadParameterPage> &pages, ofxMidiDevice *midiDevice, const ofJson &json)
{
	for (auto &page : json["pages"]) {
		std::string name = page["name"];
		MadParameterPage customPage = MadParameterPage(name, midiDevice);
		for (auto &element : page["surfaces"]) { addParameterToCustomPage(element, "surfaces", &customPage); }
		for (auto &element : page["fixtures"]) { addParameterToCustomPage(element, "fixtures", &customPage); }
		for (auto &element : page["medias"])   { addParameterToCustomPage(element, "medias",   &customPage); }
		for (auto &element : page["modules"])  { addParameterToCustomPage(element, "modules",  &customPage); }
...
void ofxMadOscQuery::addParameterToCustomPage(const ofJson &element, const std::string &type, MadParameterPage *customPage)
{
	...
	if (element.is_string()) elementName = element.get<std::string>();
	else elementName = ofToString(element).substr(1, ofToString(element).size() - 2);
...
bool ofxMadOscQuery::matchesGroupWildcard(const std::string &paramNameIn, const std::string &elementName)
{
	...
	if (paramName.find("Group") == std::string::npos) { return false; }
	...
	{
		auto seg = segList[i];
		if (!seg.empty() && seg != "Group" && elementName.find(seg) != std::string::npos) { return true; }
	}
	return false;
}
...
void ofxMadOscQuery::createSubPages(std::list<MadParameterPage> &pages, ofxMidiDevice *midiDevice, const ofJson &json)
{
	...
	for (auto &keyType : keyTypes)
	{
		...
		for (auto it = itCont->begin(); it != itCont->end(); ++it)
		{
			...
			{
				if (*itDesc == skipDescription) { shouldSkip = true; break; }
			}
			...
			if (!page.isEmpty()) { pages.push_back(page); }
		}
	}
}
...
void ofxMadOscQuery::setupPageFromJson(std::list<MadParameterPage> &pages, MadParameterPage &page, ofxMidiDevice *midiDevice, const ofJson &element, const string &keyType)
{
	...
	for (auto it = itCont->begin(); it != itCont->end(); ++it)
	{
		...
		for (auto &skipDescription : skipDescriptions)
		{
			...
			if (innerDesc != element["CONTENTS"].end() && *innerDesc == skipDescription) { return; }
		}
		...
		if (*itDesc == "Opacity")
		{
			...
			bool bIsGroup = false;
			if (keyType == "surfaces" && element["CONTENTS"].find("output") == element["CONTENTS"].end()) { bIsGroup = true; }
			else if (keyType == "fixtures" && element["CONTENTS"].size() > 10) { bIsGroup = true; }
			else return;
		...
		else if (*itDesc == "fx")
		{
			...
			if (fxCont != contents.end() && fxCont->is_object())
			{
				for (auto itFx = fxCont->begin(); itFx != fxCont->end(); ++itFx)
				{
					...
					if (fdesc != itFx.value().end() && ftype != itFx.value().end() &&
						fdesc->is_string() && ftype->is_string() && *fdesc != "FX Type" && *ftype == "f")
					{
